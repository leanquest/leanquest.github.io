...
test("the Champion catalogue prompts the player to choose a move", () => {
  assert.equal(currentModeLabel(createProofState(exercises[6], "champion"), "champion"), "CHOOSE A MOVE");
});
...
test("five basic tutorials begin both proof paths", () => {
  ...
  for (const level of exercises.slice(0, 5)) {
    replayIntendedRoute(level, "champion");
    replayIntendedRoute(level, "apprentice");
  }
...
test("the proposition tutorial accepts true and false statements before proving True", () => {
  ...
  const warriorChoices = getMoveChoices(propositionGoal, "champion", 4).map((choice) => choice.label);
  ...
  let mageGoal = move(propositionGoal, "apprentice", 4, "exact □");
  const mageChoices = getMoveChoices(mageGoal, "apprentice", 4).map((choice) => choice.label);
  ...
  assert.deepEqual(getMoveChoices(truthGoal, "champion", 5).map((choice) => choice.label), ["True.intro"]);
  mageGoal = move(truthGoal, "apprentice", 5, "exact □");
  assert.deepEqual(getMoveChoices(mageGoal, "apprentice", 5).map((choice) => choice.label), ["True.intro"]);
});

test("curriculum data controls cumulative move unlocks and lesson callouts", () => {
  assert.equal(unlockedMoves(10, "champion").has("catalogue.andIntro"), false);
  assert.equal(unlockedMoves(11, "champion").has("catalogue.andIntro"), true);
  assert.equal(unlockedMoves(11, "apprentice").has("catalogue.andIntro"), false);
  assert.equal(unlockedMoves(9, "champion").has("term.application"), true);
  assert.equal(unlockedMoves(9, "champion").has("tactic.apply"), false);
  assert.equal(unlockedMoves(9, "apprentice").has("tactic.apply"), true);
  assert.equal(unlockedMoves(13, "apprentice").has("term.dot"), true);
  assert.equal(unlockedMoves(33, "apprentice").has("tactic.rewrite"), false);
  assert.equal(unlockedMoves(34, "apprentice").has("tactic.rewrite"), true);
  assert.equal(unlockedMoves(38, "apprentice").has("tactic.subst"), false);
  assert.equal(unlockedMoves(39, "apprentice").has("tactic.subst"), true);
  assert.equal(unlockedMoves(41, "apprentice").has("tactic.contradiction"), false);
  assert.equal(unlockedMoves(42, "apprentice").has("tactic.contradiction"), true);
  assert.equal(exercises[39].title, "Chain the Equalities");
  assert.equal(unlockedMoves(39, "apprentice").has("tactic.calc"), false);
  assert.equal(unlockedMoves(40, "apprentice").has("tactic.calc"), true);
  assert.equal(unlockedMoves(44, "champion").has("catalogue.natZeroAdd"), false);
  assert.equal(unlockedMoves(45, "champion").has("catalogue.natZeroAdd"), true);
  assert.equal(unlockedMoves(50, "apprentice").has("catalogue.listLengthAppend"), false);
  assert.equal(unlockedMoves(51, "apprentice").has("catalogue.listLengthAppend"), true);
  assert.match(newMoveText(exercises[8], "champion"), /application/);
  assert.match(newMoveText(exercises[43], "champion"), /Nat\.zero_add/);
  assert.match(newMoveText(exercises[43], "apprentice"), /without using any hypothesis or catalogue theorem/);
  ...
  assert.deepEqual(lessonTextFor(exercises[0].lesson, "champion"), lessonTextFor(exercises[0].lesson, "apprentice"));

  const implicationLesson = exercises.find((exercise) => exercise.id === 7)?.lesson;
  assert.ok(implicationLesson);
  assert.match(lessonTextFor(implicationLesson, "champion")[1], /constructs the function directly/i);
  assert.match(lessonTextFor(implicationLesson, "apprentice")[1], /intro.*construct a function/i);

  const reflexivityLesson = exercises.find((exercise) => exercise.title === "Close by Reduction")?.lesson;
  assert.ok(reflexivityLesson);
  assert.equal(lessonTextFor(reflexivityLesson, "champion").length, 2);
  assert.match(lessonTextFor(reflexivityLesson, "champion")[1], /Eq\.refl/);
  assert.match(lessonTextFor(reflexivityLesson, "apprentice")[1], /rfl/);
  ...
  for (const moveId of warriorOnlyProofTerms) {
    assert.equal(unlockedMoves(55, "champion").has(moveId), true, `${moveId} must remain available to Champion`);
    assert.equal(unlockedMoves(55, "apprentice").has(moveId), false, `${moveId} must stay out of the Apprentice catalogue`);
  }
  assert.equal(unlockedMoves(55, "apprentice").has("catalogue.dataConstructors"), true);
  assert.equal(unlockedMoves(55, "apprentice").has("catalogue.natAddLeftComm"), true);
  ...
  for (const exercise of exercises) {
    for (const name of warriorOnlyNames) {
      assert.equal(exercise.apprentice.proof.includes(name), false, `Apprentice level ${exercise.id} proof must not use ${name}`);
    }
  }
  ...
  for (const [level, moveId, theorem] of theoremRewards) {
    assert.equal(unlockedMoves(level, "champion").has(moveId), false, `${theorem} must not prove itself`);
    assert.equal(unlockedMoves(level + 1, "champion").has(moveId), true, `${theorem} must unlock after completion`);
    assert.match(newMoveText(exercises[level - 1], "champion"), new RegExp(theorem.replace(".", "\\.")));
    assert.match(newMoveText(exercises[level - 1], "apprentice"), new RegExp(theorem.replace(".", "\\.")));
  }
});

test("catalogue moves expose class-appropriate mana costs", () => {
  const warriorState = createProofState("P → P", ["P : Prop"]);
  const warriorChoices = getMoveChoices(warriorState, "champion", 7);
  ...
  const intro = getMoveChoices(mageState, "apprentice", 7).find((choice) => choice.label.startsWith("intro "));
  assert.ok(intro);
  assert.equal(intro.manaCost, TACTIC_MANA_COSTS.intro);
  mageState = intro.apply();

  const exact = getMoveChoices(mageState, "apprentice", 7).find((choice) => choice.label === "exact □");
  assert.ok(exact);
  assert.equal(exact.manaCost, TACTIC_MANA_COSTS.exact);
  mageState = exact.apply();

  const argumentsChoices = getMoveChoices(mageState, "apprentice", 7);
...
test("the sum-append capstone provides sum_append through the catalogue, not the environment", () => {
  ...
  assert.equal(unlockedMoves(level.id, "champion").has("catalogue.sumAppend"), true);

  let introduced = move(state, "champion", level.id, "fun xs => □");
  introduced = move(introduced, "champion", level.id, "fun ys => □");
  introduced = move(introduced, "champion", level.id, "(□ □)");
  introduced = move(introduced, "champion", level.id, "(□ □)");
  introduced = move(introduced, "champion", level.id, "Eq.trans");
  introduced = move(introduced, "champion", level.id, "(□ □)");
  introduced = move(introduced, "champion", level.id, "(□ □)");
  assert.ok(getMoveChoices(introduced, "champion", level.id).some((choice) => choice.label === "sum_append"));
});

test("class-aware story sequences are placed and rendered entirely from curriculum data", () => {
  assert.equal(curriculum[0].kind, "story");
  assert.equal(curriculum[0].id, "the-hollow");
  assert.equal(curriculum[0].panels.length, 5);

  const level43Index = curriculum.findIndex((entry) => entry.kind === "level" && entry.id === 43);
  assert.equal(curriculum[level43Index + 1].id, "induction-apprentice");
  assert.equal(curriculum[level43Index + 1].hero, "apprentice");
  assert.equal(curriculum[level43Index + 2].id, "induction-champion");
  assert.equal(curriculum[level43Index + 2].hero, "champion");
  assert.equal(curriculum[level43Index + 3].kind, "level");
  assert.equal(curriculum[level43Index + 3].id, 44);
...
test("every level specifies its monster sprite and class hue shifts", () => {
  for (const exercise of exercises) {
    for (const hero of ["champion", "apprentice"]) {
  ...
  assert.deepEqual(exercises[35].monster.hueShift, { champion: 92, apprentice: 126 });
});
...
test("every intended proof route replays and the active economy is balanced from it", () => {
  ...
  assert.deepEqual(MAX_HP, { champion: 480, apprentice: 210 });
  assert.deepEqual(MAX_MANA, { champion: 0, apprentice: 30 });
  assert.deepEqual(TARGET_RESERVE_MOVES, { champion: 4, apprentice: 2 });
  ...
  for (const level of exercises) {
    for (const hero of ["champion", "apprentice"]) {
      ...
      if (hero === "apprentice") {
        assert.ok(route.mana <= MAX_MANA.apprentice, `Apprentice level ${level.id} costs ${route.mana} MP`);
        manaByLevel.push({ level: level.id, mana: route.mana });
      }

      const difficult = route.selections >= (hero === "champion" ? 10 : 5);
      if (difficult) {
  ...
  assert.equal(Math.max(...manaByLevel.filter(({ level }) => level >= 51).map(({ mana }) => mana)), MAX_MANA.apprentice);
});

test("champion vision normalizes the focused hole without making a proof move", () => {
...
test("champion vision preserves a substituted list constructor as one AST argument", () => {
  const state = createProofState(
...
test("completed argument placeholders cannot reappear as selectable terms", () => {
  ...
  for (const label of moves) {
    const choice = getMoveChoices(state, "champion", level.id).find((candidate) => candidate.label === label);
    assert.ok(choice, `Expected ${label} while replaying the stale-placeholder regression`);
    state = choice.apply();
  }

  const labels = getMoveChoices(state, "champion", level.id).map((choice) => choice.label);
...
test("dependent catalogue recursors are inferred from the target without environment evidence", () => {
  ...
  state = move(state, "champion", 52, "fun xs => □");
  state = move(state, "champion", 52, "fun ys => □");
  for (let index = 0; index < 4; index += 1) state = move(state, "champion", 52, "(□ □)");

  assert.equal(currentTarget(state), "(?3 → ?2 → ?1 → ?0 → List.Perm xs ys → sum xs = sum ys)");
  assert.ok(getMoveChoices(state, "champion", 52).some((choice) => choice.label === "List.Perm.rec"));
  state = move(state, "champion", 52, "List.Perm.rec");
...
test("congrArg infers a shared unary function from an equality target", () => {
  ...
  state = move(state, "champion", 52, "(□ □)");
  state = move(state, "champion", 52, "(□ □)");

  assert.ok(getMoveChoices(state, "champion", 52).some((choice) => choice.label === "congrArg"));
  state = move(state, "champion", 52, "congrArg");
  assert.equal(currentTarget(state), "Nat → ?3");
  assert.ok(
    getMoveChoices(state, "champion", 52)
...
test("the list-sum capstones use no more than two nested recursors", () => {
  const recursorCounts = exercises.slice(50).map((level) =>
    level.champion.proof.match(/(?:Nat|List|List\.Perm)\.rec/g)?.length ?? 0
  );
  assert.deepEqual(recursorCounts, [1, 1, 0, 1, 1]);

  let appendState = createProofState(exercises[50].theorem, exercises[50].environment);
  appendState = move(appendState, "apprentice", 51, "intro xs");
  appendState = move(appendState, "apprentice", 51, "intro ys");
  const appendChoices = getMoveChoices(appendState, "apprentice", 51).map((choice) => choice.label);
  ...
  replicateState = move(replicateState, "apprentice", 54, "intro n");
  replicateState = move(replicateState, "apprentice", 54, "intro x");
  assert.equal(getMoveChoices(replicateState, "apprentice", 54).some((choice) => choice.label === "simp"), false);
  assert.ok(getMoveChoices(replicateState, "apprentice", 54).some((choice) => choice.label === "induction □"));

  assert.deepEqual(
    exercises[54].apprentice.selections.filter((move) => move.startsWith("induction ")),
    ["induction □"],
  );
});

test("all five Apprentice capstones replay with their intended move and mana counts", () => {
  const routes = [
    { labels: exercises[50].apprentice.selections, mana: 24 },
    { labels: exercises[51].apprentice.selections, mana: 30 },
    { labels: exercises[52].apprentice.selections, mana: 12 },
    { labels: exercises[53].apprentice.selections, mana: 27 },
    { labels: exercises[54].apprentice.selections, mana: 27 },
  ];
  ...
  for (const [index, route] of routes.entries()) {
    ...
    for (const label of route.labels) {
      const choice = getMoveChoices(state, "apprentice", level.id).find((candidate) => candidate.label === label);
      assert.ok(choice, `Expected ${label} on Apprentice level ${level.id}`);
      mana += choice.manaCost;
      state = choice.apply();
    }
    assert.equal(isSolved(state), true, `Apprentice level ${level.id} should be solved`);
    assert.equal(mana, route.mana);
  }
});

test("Apprentice induction chapter routes use explicit facts after restricted simp", () => {
  for (const level of exercises.slice(38, 45)) {
    let state = createProofState(level.theorem, level.environment);
    for (const label of level.apprentice.selections) {
      state = move(state, "apprentice", level.id, label);
    }
    assert.equal(isSolved(state), true, `Apprentice level ${level.id} should be solved`);
  }
});

test("all five Champion capstones replay with their intended selection counts", () => {
  const replay = (levelIndex, build) => {
    const level = exercises[levelIndex];
    let state = createProofState(level.theorem, level.environment);
    let selections = 0;
    const exact = (label) => {
      const choices = getMoveChoices(state, "champion", level.id);
      const choice = choices.find((candidate) => candidate.label === label);
      assert.ok(choice, `Expected ${label} on Champion level ${level.id}; found ${choices.map((item) => item.label)}`);
      state = choice.apply();
      selections += 1;
    };
    const lambda = () => {
      const choice = getMoveChoices(state, "champion", level.id).find((candidate) => candidate.label.startsWith("fun "));
      assert.ok(choice, `Expected a lambda on Champion level ${level.id}`);
      exact(choice.label);
    };
    const direct = () => {
      const choice = getMoveChoices(state, "champion", level.id).find((candidate) =>
        candidate.label !== "(□ □)" && !candidate.label.startsWith("fun ")
      );
      assert.ok(choice, `Expected a direct hypothesis on Champion level ${level.id}`);
      exact(choice.label);
    };
    build({ exact, lambda, direct });
    assert.equal(isSolved(state), true, `Champion level ${level.id} should be solved`);
    return selections;
  };
...
test("an environment term can close the first champion level", () => {
  let state = createProofState("P", ["P : Prop", "hp : P"]);
  state = move(state, "champion", 6, "hp");
  assert.equal(isSolved(state), true);
  assert.equal(renderProof(state), "hp");
});

test("intro extends the apprentice environment before exact chooses a term", () => {
  let state = createProofState("P → P", ["P : Prop"]);
  state = move(state, "apprentice", 7, "intro hP");
  assert.ok(environmentLines(state).includes("hP : P"));
  state = move(state, "apprentice", 7, "exact □");
  assert.deepEqual(state.tacticScript, ["intro hP", "exact □"]);
  state = move(state, "apprentice", 7, "hP");
  assert.equal(isSolved(state), true);
  assert.deepEqual(state.tacticScript, ["intro hP", "exact hP"]);
  assert.deepEqual(state.moves, ["intro hP", "exact □", "hP"]);
});

test("apprentice tactic arguments fill their scroll holes without disappearing from move history", () => {
  let state = createProofState("P ∧ Q → P", ["P Q : Prop"]);
  state = move(state, "apprentice", 12, "intro h");
  state = move(state, "apprentice", 12, "apply □");
  state = move(state, "apprentice", 12, "And.left");
  assert.deepEqual(state.tacticScript, ["intro h", "apply And.left"]);
  assert.deepEqual(state.moves, ["intro h", "apply □", "And.left"]);
});

test("apprentice apply offers function-valued conjunction projections from the environment", () => {
  let state = createProofState("P ∧ ¬P → False", ["P : Prop"]);
  state = move(state, "apprentice", 19, "intro h");
  state = move(state, "apprentice", 19, "apply □");
  assert.ok(getMoveChoices(state, "apprentice", 19).some((choice) => choice.label === "h.right"));
  state = move(state, "apprentice", 19, "h.right");
  assert.equal(currentTarget(state), "P");
  state = move(state, "apprentice", 19, "exact □");
  assert.deepEqual(getMoveChoices(state, "apprentice", 19).map((choice) => choice.label), ["h.left"]);
  state = move(state, "apprentice", 19, "h.left");
  assert.equal(isSolved(state), true);
...
test("apprentice apply offers function-valued equivalence projections from the environment", () => {
  let state = createProofState("(P ↔ Q) → P → Q", ["P Q : Prop"]);
  state = move(state, "apprentice", 18, "intro h");
  state = move(state, "apprentice", 18, "intro hP");
  state = move(state, "apprentice", 18, "apply □");
  assert.equal(pendingArgumentType(state), "?₁ → … → ?ₙ → Q");
  const forwardProjection = getMoveChoices(state, "apprentice", 18).find((choice) => choice.label === "h.mp");
  assert.equal(forwardProjection?.argumentType, "P → Q");
  state = move(state, "apprentice", 18, "h.mp");
  assert.equal(currentTarget(state), "P");
  state = move(state, "apprentice", 18, "exact □");
  state = move(state, "apprentice", 18, "hP");
  assert.equal(isSolved(state), true);
});

test("apprentice term-taking tactics stage and type-check their arguments", () => {
  let state = createProofState("a = b → b = c → a = c", ["α : Type", "a b c : α"]);
  state = move(state, "apprentice", 32, "intro h");
  state = move(state, "apprentice", 32, "intro h2");

  const tacticLabels = getMoveChoices(state, "apprentice", 32).map((choice) => choice.label);
  assert.ok(tacticLabels.includes("trans □"));
  assert.equal(tacticLabels.some((label) => label.startsWith("trans ") && label !== "trans □"), false);

  state = move(state, "apprentice", 32, "trans □");
  assert.equal(pendingArgumentType(state), "α");
  assert.deepEqual(
    getMoveChoices(state, "apprentice", 32).map((choice) => [choice.label, choice.argumentType]),
    [["b", "α"]],
  );
  state = move(state, "apprentice", 32, "b");
  assert.equal(currentTarget(state), "a = b");
  assert.deepEqual(renderTacticProofLines(state), ["by", "  intro h", "  intro h2", "  trans b"]);
});

test("apprentice structural and equality tactics do not preselect environment terms", () => {
  let casesState = createProofState("(P ∨ Q) → R", ["P Q R : Prop"]);
  casesState = move(casesState, "apprentice", 15, "intro h");
  casesState = move(casesState, "apprentice", 15, "cases □");
  assert.equal(pendingArgumentType(casesState), "P ∨ Q");
  assert.deepEqual(
    getMoveChoices(casesState, "apprentice", 15).map((choice) => [choice.label, choice.argumentType]),
    [["h", "P ∨ Q"]],
  );
  ...
  rcasesState = move(rcasesState, "apprentice", 29, "intro h");
  const rcases = getMoveChoices(rcasesState, "apprentice", 29).find((choice) => choice.label === "rcases □");
  assert.ok(rcases);
  rcasesState = rcases.apply();
  assert.equal(pendingArgumentType(rcasesState), "∃ x : α, P x");
  assert.deepEqual(getMoveChoices(rcasesState, "apprentice", 29).map((choice) => choice.label), ["h"]);

  let substState = createProofState("a = b → P a → P b", [
    "α : Type", "P : α → Prop", "a b : α",
  ]);
  substState = move(substState, "apprentice", 39, "intro h");
  substState = move(substState, "apprentice", 39, "intro h2");
  substState = move(substState, "apprentice", 39, "subst □");
  assert.equal(pendingArgumentType(substState), "α");
  assert.deepEqual(
    getMoveChoices(substState, "apprentice", 39).map((choice) => [choice.label, choice.argumentType]),
    [["a", "α"], ["b", "α"]],
  );
  substState = move(substState, "apprentice", 39, "b");
  assert.equal(currentTarget(substState), "P a");
  substState = move(substState, "apprentice", 39, "exact □");
  substState = move(substState, "apprentice", 39, "h2");
  assert.equal(isSolved(substState), true);

  let calcState = createProofState("a = b → b = c → f a = f c", [
    "α β : Type", "f : α → β", "a b c : α",
  ]);
  calcState = move(calcState, "apprentice", 40, "intro h");
  calcState = move(calcState, "apprentice", 40, "intro h2");
  calcState = move(calcState, "apprentice", 40, "calc … = □ := □");
  assert.equal(pendingArgumentType(calcState), "β");
  assert.deepEqual(
    getMoveChoices(calcState, "apprentice", 40).map((choice) => [choice.label, choice.argumentType]),
    [["f b", "β"]],
  );
  calcState = move(calcState, "apprentice", 40, "f b");
  ...
  byCasesState = move(byCasesState, "apprentice", 41, "by_cases □");
  assert.equal(pendingArgumentType(byCasesState), "Prop");
  assert.deepEqual(
    getMoveChoices(byCasesState, "apprentice", 41).map((choice) => [choice.label, choice.argumentType]),
    [["P", "Prop"]],
  );
});

test("apprentice subst offers every eliminable equality variable and rewrites the whole proof state", () => {
  let state = createProofState("a = b → b = c → f a = f c", [
    "α β : Type", "f : α → β", "a b c : α",
  ]);
  state = move(state, "apprentice", 40, "intro h");
  state = move(state, "apprentice", 40, "intro h2");
  state = move(state, "apprentice", 40, "subst □");

  assert.equal(pendingArgumentType(state), "α");
  assert.deepEqual(
    getMoveChoices(state, "apprentice", 40).map((choice) => [choice.label, choice.argumentType]),
    [["a", "α"], ["b", "α"], ["c", "α"]],
  );

  const eliminateA = move(state, "apprentice", 40, "a");
  ...
  const eliminateB = move(state, "apprentice", 40, "b");
  ...
  const eliminateC = move(state, "apprentice", 40, "c");
...
test("apprentice learns rewriting before substitution on the existing equality levels", () => {
  let rewriteState = createProofState("a = b → P a → P b", [
    "α : Type", "P : α → Prop", "a b : α",
  ]);
  rewriteState = move(rewriteState, "apprentice", 34, "intro h");
  rewriteState = move(rewriteState, "apprentice", 34, "intro h2");
  assert.equal(getMoveChoices(rewriteState, "apprentice", 34).some((choice) => choice.label === "subst □"), false);
  rewriteState = move(rewriteState, "apprentice", 34, "rw [← □]");
  rewriteState = move(rewriteState, "apprentice", 34, "h");
  rewriteState = move(rewriteState, "apprentice", 34, "exact □");
  rewriteState = move(rewriteState, "apprentice", 34, "h2");
  ...
  substState = move(substState, "apprentice", 39, "rfl");
  assert.equal(isSolved(substState), true);
});

test("apprentice proof scroll renders calc steps as Lean syntax", () => {
  let state = createProofState("a = b → b = c → f a = f c", [
    "α β : Type", "f : α → β", "a b c : α",
  ]);
  state = move(state, "apprentice", 40, "intro h");
  state = move(state, "apprentice", 40, "intro h2");
  state = move(state, "apprentice", 40, "calc … = □ := □");
  state = move(state, "apprentice", 40, "f b");
  state = move(state, "apprentice", 40, "congr");
  state = move(state, "apprentice", 40, "exact □");
  state = move(state, "apprentice", 40, "h");
  state = move(state, "apprentice", 40, "congr");
  state = move(state, "apprentice", 40, "exact □");
  state = move(state, "apprentice", 40, "h2");
...
test("apprentice proof scroll nests calc syntax inside an ordinary tactic branch", () => {
  let state = createProofState("a = b → b = c → P → f a = f c ∧ P", [
    "α β : Type", "f : α → β", "a b c : α", "P : Prop",
  ]);
  state = move(state, "apprentice", 40, "intro h");
  state = move(state, "apprentice", 40, "intro h2");
  state = move(state, "apprentice", 40, "intro hP");
  state = move(state, "apprentice", 40, "constructor");
  state = move(state, "apprentice", 40, "calc … = □ := □");
  state = move(state, "apprentice", 40, "f b");
  state = move(state, "apprentice", 40, "congr");
  state = move(state, "apprentice", 40, "exact □");
  ...
  state = move(state, "apprentice", 40, "h2");
  state = move(state, "apprentice", 40, "exact □");
  state = move(state, "apprentice", 40, "hP");
...
test("apprentice proof scroll marks the goals created by constructor with bullets", () => {
  let state = createProofState("P ∧ Q → Q ∧ P", ["P Q : Prop"]);
  state = move(state, "apprentice", 13, "intro h");
  state = move(state, "apprentice", 13, "constructor");
  state = move(state, "apprentice", 13, "exact □");
  state = move(state, "apprentice", 13, "h.right");
  state = move(state, "apprentice", 13, "exact □");
  state = move(state, "apprentice", 13, "h.left");
...
test("apprentice proof scroll indents tactics and bullets inside nested constructors", () => {
  let state = createProofState("P → Q → R → P ∧ (Q ∧ R)", ["P Q R : Prop"]);
  state = move(state, "apprentice", 36, "intro hP");
  state = move(state, "apprentice", 36, "intro hQ");
  state = move(state, "apprentice", 36, "intro hR");
  state = move(state, "apprentice", 36, "constructor");
  state = move(state, "apprentice", 36, "exact □");
  state = move(state, "apprentice", 36, "hP");
  state = move(state, "apprentice", 36, "constructor");
  state = move(state, "apprentice", 36, "exact □");
  state = move(state, "apprentice", 36, "hQ");
  state = move(state, "apprentice", 36, "exact □");
  state = move(state, "apprentice", 36, "hR");
...
test("apprentice proof scroll marks every branch created by cases", () => {
  let state = createProofState("(P → R) → (Q → R) → P ∨ Q → R", ["P Q R : Prop"]);
  state = move(state, "apprentice", 15, "intro hPR");
  state = move(state, "apprentice", 15, "intro hQR");
  state = move(state, "apprentice", 15, "intro h");
  state = move(state, "apprentice", 15, "cases □");
  state = move(state, "apprentice", 15, "h");
  state = move(state, "apprentice", 15, "apply □");
  state = move(state, "apprentice", 15, "hPR");
  state = move(state, "apprentice", 15, "exact □");
  state = move(state, "apprentice", 15, "hP");
  state = move(state, "apprentice", 15, "apply □");
  state = move(state, "apprentice", 15, "hQR");
  state = move(state, "apprentice", 15, "exact □");
  state = move(state, "apprentice", 15, "hQ");
...
test("apprentice proof scroll marks every branch created by induction", () => {
  let state = createProofState("∀ n : Nat, 0 + n = n", []);
  state = move(state, "apprentice", 44, "intro n");
  state = move(state, "apprentice", 44, "induction □");
  state = move(state, "apprentice", 44, "n");
  state = move(state, "apprentice", 44, "rfl");
  state = move(state, "apprentice", 44, "simp");
  state = move(state, "apprentice", 44, "congr");
  state = move(state, "apprentice", 44, "exact □");
  state = move(state, "apprentice", 44, "ih");
...
test("apprentice simp performs reductions without using hypotheses or catalogue theorems", () => {
  const factGoal = createProofState("0 + n = n", [
    "n : Nat",
    "ih : 0 + n = n",
  ]);
  assert.equal(unlockedMoves(47, "apprentice").has("catalogue.natZeroAdd"), true);
  assert.equal(
    getMoveChoices(factGoal, "apprentice", 47).some((choice) => choice.label === "simp"),
    false,
  );
  ...
  reduced = move(reduced, "apprentice", 47, "simp");
  ...
  trivial = move(trivial, "apprentice", 44, "simp");
  assert.equal(isSolved(trivial), true);
});

test("apprentice proof scroll marks multiple goals created after choosing an apply function", () => {
  let state = createProofState("(P → Q → R) → R", ["P Q R : Prop", "hP : P", "hQ : Q"]);
  state = move(state, "apprentice", 15, "intro hPQR");
  state = move(state, "apprentice", 15, "apply □");
  state = move(state, "apprentice", 15, "hPQR");
  state = move(state, "apprentice", 15, "exact □");
  state = move(state, "apprentice", 15, "hP");
  state = move(state, "apprentice", 15, "exact □");
  state = move(state, "apprentice", 15, "hQ");
...
test("nested applications fill the focused hole instead of appending text", () => {
  ...
  state = move(state, "champion", 10, "fun hPQ => □");
  state = move(state, "champion", 10, "fun hQR => □");
  state = move(state, "champion", 10, "fun hP => □");
  state = move(state, "champion", 10, "(□ □)");
  assert.equal(currentTarget(state), "(?0 → R)");
  assert.deepEqual(
    renderProofParts(state).filter((part) => part.hole).map((part) => part.active),
    [true, false],
  );
  state = move(state, "champion", 10, "hQR");
  state = move(state, "champion", 10, "(□ □)");
  state = move(state, "champion", 10, "hPQ");
  state = move(state, "champion", 10, "hP");
  assert.equal(isSolved(state), true);
...
test("a multi-argument catalogue function is built with ordinary applications", () => {
  let state = createProofState("P → Q → P ∧ Q", ["P Q : Prop"]);
  state = move(state, "champion", 11, "fun hP => □");
  state = move(state, "champion", 11, "fun hQ => □");
  state = move(state, "champion", 11, "(□ □)");
  state = move(state, "champion", 11, "(□ □)");
  state = move(state, "champion", 11, "And.intro");
  assert.deepEqual(
    renderProofParts(state).filter((part) => part.hole).map((part) => part.active),
    [true, false],
  );
  state = move(state, "champion", 11, "hP");
  state = move(state, "champion", 11, "hQ");
...
test("constructor functions are catalogue terms rather than pre-applied moves", () => {
  let state = createProofState("P → Q → P ∧ Q", ["P Q : Prop"]);
  assert.equal(getMoveChoices(state, "champion", 11).some((move) => move.label.includes("And.intro □")), false);
  assert.equal(getMoveChoices(state, "apprentice", 11).some((move) => move.label === "constructor"), false);

  state = move(state, "champion", 11, "fun hP => □");
  state = move(state, "champion", 11, "fun hQ => □");
  assert.deepEqual(getMoveChoices(state, "champion", 11).map((move) => move.label), ["(□ □)"]);
  state = move(state, "champion", 11, "(□ □)");
  state = move(state, "champion", 11, "(□ □)");
  assert.ok(getMoveChoices(state, "champion", 11).some((move) => move.label === "And.intro"));
});
...
test("catalogue functions never arrive with pre-filled argument holes", () => {
  ...
  for (const item of cases) {
    ...
    const labels = getMoveChoices(state, "champion", item.level).map((choice) => choice.label);
...
test("dependent applications instantiate later argument types from the chosen term", () => {
  ...
  state = move(state, "champion", 28, "fun h => □");
  state = move(state, "champion", 28, "(□ □)");
  state = move(state, "champion", 28, "(□ □)");
  state = move(state, "champion", 28, "Exists.intro");
  assert.equal(currentTarget(state), "α");
  state = move(state, "champion", 28, "a");
  assert.equal(currentTarget(state), "P a");
  state = move(state, "champion", 28, "h");
...
test("the first tutorial unlocks natural number entry for terms and tactic arguments", () => {
  ...
  assert.equal(
    getMoveChoices(locked, "champion", 0).some((choice) => choice.label === "natural number"),
    false,
  );
  assert.equal(
    getMoveChoices(locked, "champion", 1).some((choice) => choice.label === "natural number"),
    true,
  );

  let champion = createProofState("∃ n : Nat, n = 0", []);
  champion = move(champion, "champion", 30, "(□ □)");
  champion = move(champion, "champion", 30, "(□ □)");
  champion = move(champion, "champion", 30, "Exists.intro");
  champion = inputMove(champion, "champion", 30, "natural number", "000");
  assert.equal(currentTarget(champion), "0 = 0");
  champion = move(champion, "champion", 30, "(□ □)");
  champion = move(champion, "champion", 30, "Eq.refl");
  champion = inputMove(champion, "champion", 30, "natural number", "0");
  assert.equal(renderProof(champion), "Exists.intro 0 (Eq.refl 0)");
  assert.equal(isSolved(champion), true);

  let apprentice = createProofState("∃ n : Nat, n = 0", []);
  apprentice = move(apprentice, "apprentice", 30, "use □");
  apprentice = inputMove(apprentice, "apprentice", 30, "natural number", "0");
  assert.equal(currentTarget(apprentice), "0 = 0");
  apprentice = move(apprentice, "apprentice", 30, "rfl");
  assert.equal(isSolved(apprentice), true);
});

test("natural-number entry rejects malformed values without changing proof state", () => {
  const state = createProofState("Nat", []);
  const choice = getMoveChoices(state, "champion", 1)
...
test("proposition eliminators are not offered when the current goal is data", () => {
  ...
  const dataLabels = getMoveChoices(createProofState("Nat", environment), "apprentice", 55)
  ...
  const propositionLabels = getMoveChoices(createProofState("P", environment), "apprentice", 55)
    .map((choice) => choice.label);
...
test("proposition-only catalogue constants cannot be applied to produce data", () => {
  ...
  state = move(state, "champion", 55, "(□ □)");
  let labels = getMoveChoices(state, "champion", 55).map((choice) => choice.label);
  ...
  state = move(state, "champion", 55, "(□ □)");
  labels = getMoveChoices(state, "champion", 55).map((choice) => choice.label);
...
test("level seven teaches And.left through ordinary application", () => {
  let state = createProofState("P ∧ Q → P", ["P Q : Prop"]);
  const openingLabels = getMoveChoices(state, "champion", 12).map((choice) => choice.label);
  assert.ok(openingLabels.includes("And.left"));
  assert.equal(openingLabels.includes("And.left □"), false);

  const direct = move(state, "champion", 12, "And.left");
  assert.equal(renderProof(direct), "And.left");
  assert.equal(isSolved(direct), true);

  state = move(state, "champion", 12, "fun h => □");
  const initialLabels = getMoveChoices(state, "champion", 12).map((choice) => choice.label);
  ...
  state = move(state, "champion", 12, "(□ □)");
  assert.ok(getMoveChoices(state, "champion", 12).some((choice) => choice.label === "And.left"));
  state = move(state, "champion", 12, "And.left");
  assert.ok(getMoveChoices(state, "champion", 12).some((choice) => choice.label === "h"));
  state = move(state, "champion", 12, "h");
...
test("goal-derived projection functions appear before their inputs are introduced", () => {
  ...
  assert.ok(getMoveChoices(rightState, "champion", 13).some((choice) => choice.label === "And.right"));

  const iffState = createProofState("(P ↔ Q) → P → Q", ["P Q : Prop"]);
  assert.ok(getMoveChoices(iffState, "champion", 18).some((choice) => choice.label === "Iff.mp"));
});

test("level eight dot notation fills its source and function separately", () => {
  let state = createProofState("P ∧ Q → P", ["P Q : Prop"]);
  state = move(state, "champion", 13, "fun h => □");
  const initialLabels = getMoveChoices(state, "champion", 13).map((choice) => choice.label);
  assert.ok(initialLabels.includes("□.□"));
  assert.equal(initialLabels.includes("h.left"), false);

  state = move(state, "champion", 13, "□.□");
  assert.equal(renderProof(state), "fun h => □.□");
  assert.deepEqual(getMoveChoices(state, "champion", 13).map((choice) => choice.label), ["h"]);
  state = move(state, "champion", 13, "h");
  assert.equal(renderProof(state), "fun h => h.□");
  assert.deepEqual(getMoveChoices(state, "champion", 13).map((choice) => choice.label), ["left"]);
  state = move(state, "champion", 13, "left");
...
test("dot notation excludes arbitrary namespaced functions", () => {
  ...
  const labels = getMoveChoices(state, "champion", 13).map((choice) => choice.label);
...
test("dot notation retains both Iff projections", () => {
  let state = createProofState("(P ↔ Q) → (P → Q)", ["P Q : Prop"]);
  state = move(state, "champion", 18, "fun h => □");
  state = move(state, "champion", 18, "□.□");
  state = move(state, "champion", 18, "h");
  state = move(state, "champion", 18, "mp");
  ...
  state = move(state, "champion", 18, "□.□");
  state = move(state, "champion", 18, "h");
  state = move(state, "champion", 18, "mpr");
...
test("application metavariables resolve across linked holes on level fourteen", () => {
  let state = createProofState("P ∧ ¬P → False", ["P : Prop"]);
  state = move(state, "champion", 19, "fun h => □");
  state = move(state, "champion", 19, "(□ □)");

  assert.equal(currentTarget(state), "(?0 → False)");
  assert.ok(getMoveChoices(state, "champion", 19).some((choice) => choice.label.startsWith("fun ")));

  state = move(state, "champion", 19, "□.□");
  state = move(state, "champion", 19, "h");
  state = move(state, "champion", 19, "right");

  assert.equal(currentTarget(state), "P");
  state = move(state, "champion", 19, "□.□");
  state = move(state, "champion", 19, "h");
  state = move(state, "champion", 19, "left");
...
test("Or.elim is supplied through three ordinary applications", () => {
  ...
  state = move(state, "champion", 15, "fun hPR => □");
  state = move(state, "champion", 15, "fun hQR => □");
  state = move(state, "champion", 15, "fun h => □");
  state = move(state, "champion", 15, "(□ □)");
  state = move(state, "champion", 15, "(□ □)");
  state = move(state, "champion", 15, "(□ □)");
  state = move(state, "champion", 15, "Or.elim");
  assert.equal(renderProof(state), "fun hPR => fun hQR => fun h => Or.elim □ □ □");
  assert.deepEqual(
    renderProofParts(state).filter((part) => part.hole).map((part) => part.active),
    [true, false, false],
  );
  assert.ok(getMoveChoices(state, "champion", 15).some((choice) => choice.label === "h"));
  state = move(state, "champion", 15, "h");
  state = move(state, "champion", 15, "hPR");
  state = move(state, "champion", 15, "hQR");
...
test("Or.elim introduces fresh alternatives resolved by its disjunction argument", () => {
  ...
  state = move(state, "champion", 37, "fun h => □");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "Or.elim");
  assert.equal(currentTarget(state), "?3 ∨ ?4");
  state = move(state, "champion", 37, "h");
  assert.equal(currentTarget(state), "(P ∨ Q) → R ∨ Q ∨ P");
  state = move(state, "champion", 37, "fun h2 => □");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "(□ □)");
  state = move(state, "champion", 37, "Or.elim");
  state = move(state, "champion", 37, "h2");
  assert.equal(currentTarget(state), "P → R ∨ Q ∨ P");
});

test("proof rendering keeps parentheses around a lambda used as a function", () => {
  let state = createProofState("P", ["P : Prop", "hp : P"]);
  state = move(state, "champion", 9, "(□ □)");
  state = move(state, "champion", 9, "fun h => □");
  state = move(state, "champion", 9, "hp");
  state = move(state, "champion", 9, "hp");
...
test("dependent universal application infers and filters its argument", () => {
  ...
  state = move(state, "champion", 26, "fun h_Px => □");
  state = move(state, "champion", 26, "(□ □)");
  state = move(state, "champion", 26, "h_Px");
  assert.ok(getMoveChoices(state, "champion", 26).some((item) => item.label === "a"));
  state = move(state, "champion", 26, "a");
  assert.equal(isSolved(state), true);
});

test("Eq.refl matches a goal modulo definitional equality on level thirty-eight", () => {
  let state = createProofState("∀ n : Nat, n + 0 = n", []);
  state = move(state, "champion", 43, "fun n => □");
  state = move(state, "champion", 43, "(□ □)");
  assert.ok(getMoveChoices(state, "champion", 43).some((choice) => choice.label === "Eq.refl"));
  state = move(state, "champion", 43, "Eq.refl");
  assert.equal(currentTarget(state), "Nat");
  assert.deepEqual(getMoveChoices(state, "champion", 43).map((choice) => choice.label), ["n"]);
  state = move(state, "champion", 43, "n");
...
test("Eq.trans introduces a fresh middle term instead of choosing one from the environment", () => {
  ...
  state = move(state, "champion", 44, "(□ □)");
  state = move(state, "champion", 44, "(□ □)");
  state = move(state, "champion", 44, "Eq.trans");
...
test("congrArg unifies with a partially unknown equality result", () => {
  ...
  state = move(state, "champion", 44, "(□ □)");
  state = move(state, "champion", 44, "(□ □)");
  assert.ok(getMoveChoices(state, "champion", 44).some((choice) => choice.label === "congrArg"));

  state = move(state, "champion", 44, "congrArg");
  assert.equal(currentTarget(state), "Nat → Nat");
  assert.ok(getMoveChoices(state, "champion", 44).some((choice) => choice.label === "Nat.succ"));

  state = move(state, "champion", 44, "Nat.succ");
...
test("catalogue unification normalizes recursive terms before offering congrArg", () => {
  ...
  state = move(state, "champion", 44, "ih");
...
test("congrArg is offered for a normalized list constructor application", () => {
  ...
  state = move(state, "champion", 48, "(□ □)");
  state = move(state, "champion", 48, "(□ □)");

  assert.ok(getMoveChoices(state, "champion", 48).some((choice) => choice.label === "congrArg"));
  state = move(state, "champion", 48, "congrArg");
  assert.equal(currentTarget(state), "List α → List α");
  assert.ok(getMoveChoices(state, "champion", 48).some((choice) => choice.label === "List.cons head"));
  state = move(state, "champion", 48, "List.cons head");
  assert.match(currentTarget(state), /^tail \+\+ \[\] = \?\d+$/);
  state = move(state, "champion", 48, "h");
...
test("Eq.refl sees the definitional reduction of List.length on level forty-five", () => {
  ...
  state = move(state, "champion", 50, "(□ □)");

  assert.ok(getMoveChoices(state, "champion", 50).some((choice) => choice.label === "Eq.refl"));
  state = move(state, "champion", 50, "Eq.refl");
  assert.equal(currentTarget(state), "Nat");
  assert.ok(getMoveChoices(state, "champion", 50).some((choice) => choice.label === "0"));
  state = move(state, "champion", 50, "0");
...
test("a level forty-five hypothesis unifies across length notation and application form", () => {
  ...
  assert.ok(getMoveChoices(state, "champion", 50).some((choice) => choice.label === "h"));
  const solved = move(state, "champion", 50, "h");
...
test("unification normalizes terms inside applications and list expressions", () => {
  ...
  assert.ok(getMoveChoices(nested, "champion", 44).some((choice) => choice.label === "h"));

  let list = createProofState(
    "[] ++ xs = xs",
    ["α : Type", "xs : List α"],
  );
  list = move(list, "champion", 48, "(□ □)");
  assert.ok(getMoveChoices(list, "champion", 48).some((choice) => choice.label === "Eq.refl"));
});
...
test("definitional normalization follows recursive equations without algebraic reassociation", () => {
  const offersEqRefl = (theorem, environment = []) => {
    ...
    state = move(state, "champion", 48, "(□ □)");
    return getMoveChoices(state, "champion", 48).some((choice) => choice.label === "Eq.refl");
  };
...
test("Eq.refl offers only the inferred normal-form argument", () => {
  ...
  state = move(state, "champion", 45, "fun n => □");
  state = move(state, "champion", 45, "fun m => □");
  state = move(state, "champion", 45, "(□ □)");
  state = move(state, "champion", 45, "Eq.refl");

  assert.deepEqual(
    getMoveChoices(state, "champion", 45).map((choice) => choice.label),
    ["Nat.succ (n + m)"],
  );
  state = move(state, "champion", 45, "Nat.succ (n + m)");
...
test("Nat.rec chooses its induction variable in the final argument", () => {
  ...
  state = move(state, "champion", 46, "fun c => □");
  state = move(state, "champion", 46, "fun b => □");
  state = move(state, "champion", 46, "fun a => □");
  state = move(state, "champion", 46, "(□ □)");
  state = move(state, "champion", 46, "(□ □)");
  state = move(state, "champion", 46, "(□ □)");
  state = move(state, "champion", 46, "Nat.rec");

  assert.equal(currentTarget(state), "Nat");
  assert.deepEqual(
    getMoveChoices(state, "champion", 46).map((choice) => choice.label),
    ["c", "b", "a"],
  );
  state = move(state, "champion", 46, "c");
...
test("Nat.rec uses its genuine dependent type before later theorem binders are introduced", () => {
  ...
  assert.ok(getMoveChoices(state, "champion", 46).some((choice) => choice.label === "Nat.rec"));
  state = move(state, "champion", 46, "Nat.rec");
  assert.deepEqual(getMoveChoices(state, "champion", 46).map((choice) => choice.label), ["c"]);
  state = move(state, "champion", 46, "c");
  assert.equal(
    currentTarget(state),
...
test("List.rec uses its genuine dependent type before the list binder is introduced", () => {
  ...
  assert.ok(getMoveChoices(state, "champion", 48).some((choice) => choice.label === "List.rec"));
  state = move(state, "champion", 48, "List.rec");
...
test("term syntax keeps list append distinct from natural addition", () => {
  ...
  for (const label of ["(□ □)", "(□ □)", "List.rec", "(□ □)", "Eq.refl"]) {
    state = move(state, "champion", 48, label);
  }
  assert.equal(renderProof(state), "List.rec (Eq.refl □) □");
  assert.equal(currentTarget(state), "List α");
  assert.deepEqual(getMoveChoices(state, "champion", 48).map((choice) => choice.label), ["[]"]);
  state = move(state, "champion", 48, "[]");
  assert.equal(
    currentTarget(state),
...
test("implicit list theorem arguments are inferred from the goal", () => {
  ...
  assert.ok(
    getMoveChoices(state, "champion", 51).some((choice) => choice.label === "List.length_append"),
  );
});

test("apprentice apply instantiates canonical implicit catalogue parameters", () => {
  let state = createProofState("P ∧ Q → P", ["P Q : Prop"]);
  state = move(state, "apprentice", 12, "intro h");
  state = move(state, "apprentice", 12, "apply □");
  state = move(state, "apprentice", 12, "And.left");
  assert.match(currentTarget(state), /^P ∧ \?\d+$/);
  state = move(state, "apprentice", 12, "exact □");
  state = move(state, "apprentice", 12, "h");
  assert.equal(isSolved(state), true);
});

test("apprentice apply jointly infers explicit dependent theorem arguments", () => {
  let state = createProofState(
    "x + (sum xs + sum ys) = x + sum xs + sum ys",
    ["x : Nat", "xs ys : List Nat"],
  );
  state = move(state, "apprentice", 51, "symm");
  state = move(state, "apprentice", 51, "apply □");
  assert.ok(getMoveChoices(state, "apprentice", 51).some((choice) => choice.label === "Nat.add_assoc"));
  state = move(state, "apprentice", 51, "Nat.add_assoc");
  assert.equal(isSolved(state), true);
  assert.deepEqual(renderTacticProofLines(state), ["by", "  symm", "  apply Nat.add_assoc"]);
});

test("apprentice apply offers only arithmetic theorems whose conclusions strictly unify", () => {
  let state = createProofState(
    "sum l + y = y + sum l",
    ["x y : Nat", "l : List Nat"],
  );
  state = move(state, "apprentice", 52, "apply □");
  const labels = getMoveChoices(state, "apprentice", 52).map((choice) => choice.label);
...
test("ordinary metavariables can be function-typed and solved by application", () => {
  ...
  state = move(state, "champion", 46, "h");
...
test("application arguments are inferred jointly across dependent binders", () => {
  ...
  assert.ok(getMoveChoices(state, "champion", 46).some((choice) => choice.label === "hatbn"));
  state = move(state, "champion", 46, "hatbn");
  assert.deepEqual(getMoveChoices(state, "champion", 46).map((choice) => choice.label), ["b"]);
  state = move(state, "champion", 46, "b");
  assert.deepEqual(getMoveChoices(state, "champion", 46).map((choice) => choice.label), ["a"]);
  state = move(state, "champion", 46, "a");
...
test("catalogue terms are not offered when their explicit arity cannot unify", () => {
  ...
  assert.equal(
    getMoveChoices(state, "champion", 44).some((choice) => choice.label === "congrArg"),
    false,
  );
});
...
test("other polymorphic eliminators remain generic until their arguments are supplied", () => {
  ...
  conjunction = move(conjunction, "champion", 12, "fun h => □");
  conjunction = move(conjunction, "champion", 12, "(□ □)");
  conjunction = move(conjunction, "champion", 12, "And.left");
  assert.match(currentTarget(conjunction), /^P ∧ \?\d+$/);
  conjunction = move(conjunction, "champion", 12, "h");
  assert.equal(isSolved(conjunction), true);

  let iff = createProofState("Q", ["P Q : Prop", "hIff : P ↔ Q", "hP : P"]);
  iff = move(iff, "champion", 18, "(□ □)");
  iff = move(iff, "champion", 18, "(□ □)");
  iff = move(iff, "champion", 18, "Iff.mp");
  assert.match(currentTarget(iff), /^\?\d+ ↔ Q$/);
  iff = move(iff, "champion", 18, "hIff");
  ...
  exists = move(exists, "champion", 29, "(□ □)");
  exists = move(exists, "champion", 29, "(□ □)");
  exists = move(exists, "champion", 29, "Exists.elim");
  assert.match(currentTarget(exists), /^∃ x : \?\d+, \?\d+ x$/);
  exists = move(exists, "champion", 29, "hEx");
  ...
  transport = move(transport, "champion", 34, "(□ □)");
  transport = move(transport, "champion", 34, "(□ □)");
  transport = move(transport, "champion", 34, "Eq.mp");
  assert.match(currentTarget(transport), /^\?\d+ = P b$/);
  transport = move(transport, "champion", 34, "hEq");
  assert.equal(currentTarget(transport), "P a");
});

test("the apprentice handles falsehood with tactics and a simple hypothesis term", () => {
  let state = createProofState("False → P", ["P : Prop"]);
  state = move(state, "apprentice", 16, "intro hFalse");
  state = move(state, "apprentice", 16, "exfalso");
  state = move(state, "apprentice", 16, "exact □");
  state = move(state, "apprentice", 16, "hFalse");
  assert.equal(isSolved(state), true);

  let disjunctionState = createProofState("P ∨ False → P", ["P : Prop"]);
  disjunctionState = move(disjunctionState, "apprentice", 23, "intro h");
  disjunctionState = move(disjunctionState, "apprentice", 23, "cases □");
  disjunctionState = move(disjunctionState, "apprentice", 23, "h");
  disjunctionState = move(disjunctionState, "apprentice", 23, "exact □");
  disjunctionState = move(disjunctionState, "apprentice", 23, "hP");
  assert.equal(
    getMoveChoices(disjunctionState, "apprentice", 23).some((choice) => choice.label === "contradiction"),
    false,
  );
  disjunctionState = move(disjunctionState, "apprentice", 23, "exfalso");
  disjunctionState = move(disjunctionState, "apprentice", 23, "exact □");
  disjunctionState = move(disjunctionState, "apprentice", 23, "hFalse");
  assert.equal(isSolved(disjunctionState), true);

  let contradictionState = createProofState("¬P → P → Q", ["P Q : Prop"]);
  contradictionState = move(contradictionState, "apprentice", 42, "intro hnP");
  contradictionState = move(contradictionState, "apprentice", 42, "intro hP");
  contradictionState = move(contradictionState, "apprentice", 42, "contradiction");
...
test("rewrite levels require rewriting beyond their earlier direct tactic patterns", () => {
  ...
  iffState = move(iffState, "apprentice", 38, "intro h");
  iffState = move(iffState, "apprentice", 38, "intro h2");
  assert.equal(currentTarget(iffState), "Q ∨ R");
  assert.equal(getMoveChoices(iffState, "apprentice", 38).some((choice) => choice.label === "exact □"), false);
  assert.ok(getMoveChoices(iffState, "apprentice", 38).some((choice) => choice.label === "rw [← □]"));
  iffState = move(iffState, "apprentice", 38, "rw [← □]");
  iffState = move(iffState, "apprentice", 38, "h");
  assert.equal(currentTarget(iffState), "P ∨ R");
  iffState = move(iffState, "apprentice", 38, "exact □");
  iffState = move(iffState, "apprentice", 38, "h2");
  ...
  equalityState = move(equalityState, "apprentice", 39, "intro h");
  const oldCongruenceStep = move(equalityState, "apprentice", 39, "congr");
  assert.equal(currentTarget(oldCongruenceStep), "f a = f b");
  assert.equal(
    getMoveChoices(oldCongruenceStep, "apprentice", 39).some((choice) => choice.label === "exact □"),
    false,
  );
  assert.ok(getMoveChoices(equalityState, "apprentice", 39).some((choice) => choice.label === "rw [□]"));
  equalityState = move(equalityState, "apprentice", 39, "rw [□]");
  equalityState = move(equalityState, "apprentice", 39, "h");
  equalityState = move(equalityState, "apprentice", 39, "rfl");
  ...
  assert.ok(getMoveChoices(compoundEqualityState, "apprentice", 52).some((choice) => choice.label === "rw [□]"));
  const forwardRewrite = move(compoundEqualityState, "apprentice", 52, "rw [□]");
  assert.deepEqual(getMoveChoices(forwardRewrite, "apprentice", 52).map((choice) => choice.label), ["ih1"]);
  compoundEqualityState = move(forwardRewrite, "apprentice", 52, "ih1");
  ...
  const reverseRewrite = move(reverseCompoundEqualityState, "apprentice", 52, "rw [← □]");
  assert.ok(getMoveChoices(reverseRewrite, "apprentice", 52).some((choice) => choice.label === "ih2"));
  reverseCompoundEqualityState = move(reverseRewrite, "apprentice", 52, "ih2");
  ...
  compoundPropositionState = move(compoundPropositionState, "apprentice", 52, "rw [□]");
  compoundPropositionState = move(compoundPropositionState, "apprentice", 52, "hEq");
  ...
  for (const label of [
    ...
    iffTermState = move(iffTermState, "champion", 38, label);
  }
  assert.equal(
  ...
  for (const label of [
    ...
    equalityTermState = move(equalityTermState, "champion", 39, label);
  }
